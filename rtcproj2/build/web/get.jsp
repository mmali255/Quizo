...
if(rs2.next()){
count1=rs2.getInt(1);
System.out.println(count1+"count1 max");
session.setAttribute("max",count1);
}
if(rs1.next()){
count=rs1.getInt(1);
System.out.println(count+"count min");
session.setAttribute("min",count);

}
}
if(count>0){
    String var=String.valueOf(count);
    System.out.println("This if executed before query");
PreparedStatement ps=con.prepareStatement("select * from quizques where quizname='"+name+"' and qid="+var+" ");
 System.out.println("This if executed after query");
...
System.out.print(count+"below count");
count++;
